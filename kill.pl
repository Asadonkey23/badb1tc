...
my $randsemilla = "";
for($i = 0; $i < 55; $i++) { #Burası @vals daki yazılan harfler 0, 30 arası bir cümle oluşturuyor ex:(abababab) (st3st3tg353tg!)   
...
sub socker { #socker'ı Tanımlamış
    ...
    socket(SOCK, PF_INET, SOCK_STREAM, $proto);    #####PF_INET changed == AF_INET
...
if($#ARGV > 2) {
    ...
    die("Use: ?.pl [url] [Connections] [Requests per connection] [Initial false IP (optional)]\n");
}